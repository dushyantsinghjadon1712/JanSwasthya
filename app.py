...
def analytics():
    ...
    visits = cursor.fetchone()["total_visits"]

    cursor.execute("SELECT COUNT(*) AS total_prescriptions FROM prescriptions")
    prescriptions = cursor.fetchone()["total_prescriptions"]

    cursor.execute("""
        SELECT generic_name, stock
        FROM medicines
        ORDER BY stock ASC
        LIMIT 5
    """)
...
@app.route("/medicine_usage")
def medicine_usage():

    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)

    cursor.execute("SELECT * FROM medicine_usage LIMIT 10")
